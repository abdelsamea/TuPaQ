...
% the centroid and the axis of minimum inertia.
 ...
 function [FeatureVector] = momALInew(im)
im=double(im);
...
projected1=[xPro(:,1),xPro(:,2)];
 Q1=projected1;
    ...
    A=(Q1-Location).^2;
    E_distance1= sqrt(sum(A,2))/(DALI/2);
%%
...
projected2=[xProo(:,1),xProo(:,2)];
 Q2=projected2;
    ...
    A=(Q2-Location).^2;
    E_distance2= sqrt(sum(A,2))/(DALI/2);
%%
   Q3=centroid;
      Q3= repmat(Q3,length(Location),1);

     A=(Q3-Location).^2;
     E_distance3= sqrt(sum(A,2))/(DALI/2);
  ...
  [f2,x2] = ksdensity(E_distance2);
     ...
     figure(4)
      ...
      plot(f1,'k.-')
       figure(7)
      plot(f2,'k.-')
   %  FeatureVector = SOM(FeatureSpace,3,10000,0.9);
...
%[bandwidth,FeatureVector,xmesh,cdf]=kde(FeatureSpace);
 FeatureVector =[mean(E_distance1(:))  mean(E_distance2(:))  std(E_distance3(:))];
